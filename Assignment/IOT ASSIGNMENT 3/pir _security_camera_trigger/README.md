...
GPIO21 -----| Red LED          |
            ...
            +------------------+
Working Principle
ESP32 initializes all peripherals.
PIR sensor continuously monitors motion.
Motion detection triggers the ESP32.
System verifies whether it is within the configured active hours.
If active:
Green LED turns ON with a short beep.
Yellow LED turns ON with a medium beep.
Red LED turns ON with a long beep.
Event details are displayed on the Serial Monitor.
A cooldown period of 3 seconds prevents continuous triggering.
