...
Continuous Monitoring: The ESP32 reads real-time data from the soil moisture sensor and ambient light levels via an LDR every 30 seconds.
Weather Verification: Before turning on the water pump, the system queries the OpenWeatherMap API via Wi-Fi to check for any current or forecasted rain in the specified location.
Smart Actuation:
...
Cloud Telemetry & Logging: Every decision and sensor reading is logged directly into a Google Sheet using Google Apps Script for future analysis.
Manual Override: A physical tactile button with software debouncing allows users to manually bypass the system and water the plant if necessary.
Local Display: A 16x2 I2C LCD screen provides real-time system status to the user.
